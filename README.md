...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create nfl-pool-f47a
  fly apps create nfl-pool-f47a-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app nfl-pool-f47a
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app nfl-pool-f47a-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'nfl-pool-f47a-staging' does not match app name in config file 'nfl-pool-f47a'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name nfl-pool-f47a-db
  fly postgres attach --app nfl-pool-f47a nfl-pool-f47a-db

  fly postgres create --name nfl-pool-f47a-staging-db
  fly postgres attach --app nfl-pool-f47a-staging nfl-pool-f47a-staging-db
  ```
